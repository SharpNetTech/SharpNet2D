...
""" Experiment runner for Geodesic dataset """
...
from data.geodesic import GeodesicCircle

class RunnerGeodesic(RunnerBase):
    def __init__(
        self, *,
    ...
    def setup_dataset(self) -> GeodesicCircle:
        dataset_config = self.conf.get_config("dataset")
        dataset = GeodesicCircle(**dataset_config)
    ...
    def train(self):
        ...
        for _ in tqdm(range(self.iter_step, end_iter)):
            ...
            if self.iter_step % report_freq == 0:
                tqdm.write("Running Geodesic -> " + self.base_exp_dir)
    ...
    def evaluate(self, resolution=64):
        logging.info("Starting evaluation")
        xmin = self.conf.get_float("eval.xmin")
        xmax = self.conf.get_float("eval.xmax")
        ymin = self.conf.get_float("eval.ymin")
        ymax = self.conf.get_float("eval.ymax")

        output_dir = os.path.join(self.base_exp_dir, "eval")
        os.makedirs(output_dir, exist_ok=True)

        logging.info(f"Validating value fidelity")
        field_gt = self.export_field(
            lambda x: self.dataset.forward(x, keepdim=True),
            xmin=xmin, xmax=xmax,
            ymin=ymin, ymax=ymax,
        ...
        # Compute error
        error_map = np.abs(field_out - field_gt)
        error_map_valid = error_map[~np.isnan(error_map)]
        mae = error_map_valid.mean()
        logging.info(f"The valid mean absolute error (MAE) in validate region is: {mae}")
        with open(os.path.join(output_dir, "mae.txt"), "wt+") as f:
            f.write(f"{mae}\n")

        logging.info("Exporting error map as image")
        ...
        ax.set_ylim(ymin, ymax)
        ax.set_axis_off()
        ax.imshow(error_map, origin="lower", cmap="Reds", zorder=0, extent=(xmin, xmax, ymin, ymax), vmin=0.0, vmax=5e-4)
        figure.savefig(os.path.join(output_dir, "error_map.pdf"), bbox_inches="tight", pad_inches=0.0, transparent=True, dpi=600)
        figure.savefig(os.path.join(output_dir, "error_map.png"), bbox_inches="tight", pad_inches=0.0, transparent=True, dpi=600)
        plt.close(figure)

        logging.info(f"Validating gradient fidelity")
        grad_gt = self.export_field(
            self.dataset.gradient,
            xmin=xmin, xmax=xmax,
            ymin=ymin, ymax=ymax,
            resolution=resolution,
            dim=2,
        )
        grad_out = self.export_field(
        ...
        # Compute error
        error_grad = np.abs(grad_gt - grad_out)
        error_grad_valid = error_grad[~np.any(np.isnan(error_grad), axis=-1)]
        mae_grad = error_grad_valid.mean()
        logging.info(f"The valid mean absolute error (MAE) of gradients in validate region is: {mae_grad}")
        with open(os.path.join(output_dir, "mae_grad.txt"), "wt+") as f:
            f.write(f"{mae_grad}\n")

        logging.info("Exporting error grad as image")
        error_grad = np.mean(error_grad, axis=-1)
        error_grad = np.nan_to_num(error_grad, nan=0.0)
        figure = plt.figure()
        ax: Axes = figure.add_axes([0, 0, 1, 1])
        ax.set_xlim(xmin, xmax)
        ax.set_ylim(ymin, ymax)
        ax.set_axis_off()
        ax.imshow(error_grad, origin="lower", cmap="Reds", zorder=0, extent=(xmin, xmax, ymin, ymax), vmin=0.0, vmax=0.1)
        figure.savefig(os.path.join(output_dir, "error_grad.pdf"), bbox_inches="tight", pad_inches=0.0, transparent=True, dpi=600)
        figure.savefig(os.path.join(output_dir, "error_grad.png"), bbox_inches="tight", pad_inches=0.0, transparent=True, dpi=600)
        plt.close(figure)
...
def main():
    ...
    runner = RunnerGeodesic(conf_path=args.conf, is_continue=args.is_continue, mode=args.mode)
    ...
    if args.mode == "train":
        ...
        runner.evaluate(resolution=args.resolution)
